...
{
    public async Task<Result<IEnumerable<INamedByteSourceWithPath>>> Create()
    {
    ...
    private static async Task<Result<TempKeystoreFile>> CreateTempKeystore(IByteSource byteSource)
    {
        return await Result.Try(async () =>
        {
            ...
            await byteSource.DumpTo(stream);

            return tempFile;
        });
    }

    private static IEnumerable<INamedByteSourceWithPath> ApkFiles(IDirectory directory)
    {
