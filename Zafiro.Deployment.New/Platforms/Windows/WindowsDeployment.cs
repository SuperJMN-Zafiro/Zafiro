...
public class WindowsDeployment(IDotnet dotnet, DivineBytes.Path projectPath, WindowsDeployment.DeploymentOptions options, Maybe<ILogger> logger)
{
    ...
    private Task<Result<INamedByteSourceWithPath>> CreateFor(Architecture architecture, DeploymentOptions deploymentOptions)
    {
        ...
        return dotnet.Publish(projectPath, args)
            .Bind(directory => directory.FilesWithPathsRecursive()
            ...
            .Map(INamedByteSourceWithPath (file) => new NamedByteSourceWithPath(finalName, file));
    }
